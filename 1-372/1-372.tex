...
\matrix[column sep=1cm, row sep=0cm] {
    ...
    \draw (0,0) rectangle (3.5,0.5);

    \foreach \x in {0,0.4,...,3.5}{
         \draw (\x,0.20) -- (\x,0.5);
    }
    \foreach \x in {0,0.1,...,3.5}{
        \draw (\x,0.35) -- (\x,0.5);
    }
...
&
    ...
    \draw[->, >=triangle 60] (1,2) coordinate (K') -- node [left, xshift=-1em, draw=black,circle] {$K'$} ++ (0,3); 
    \draw[force,->, >=latex] ($(K)+(0,2.5)$) -- ++ (-1,0) node [left] {$\vec{u}$};
    \draw[->, >=triangle 60] (K') - ++ (3,0); 
    \draw[line width=5pt] (K')--++(2,0);
    \draw (0,0) rectangle (3.5,0.5);

    \foreach \x in {0,0.8,...,3.5}{
      \draw (\x,0.20) -- (\x,0.5);
    }
    \foreach \x in {0,0.2,...,3.5}{
        \draw (\x,0.35) -- (\x,0.5);
    }
